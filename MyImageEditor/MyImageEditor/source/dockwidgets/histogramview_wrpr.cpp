...
void CHistogramViewWrpr::calculateHistogram(const QImage& f_image)
{
    ...
    this->addPlot(m_histogram.getNormalizedChannel0().m_xvalues, m_histogram.getNormalizedChannel0().m_yvalues, QColor(0, 0, 255));
    if(QImage::Format_Indexed8 != f_image.format())
    {
        this->addPlot(m_histogram.getNormalizedChannel1().m_xvalues, m_histogram.getNormalizedChannel1().m_yvalues, QColor(0, 255, 0));
        this->addPlot(m_histogram.getNormalizedChannel2().m_xvalues, m_histogram.getNormalizedChannel2().m_yvalues, QColor(255, 0, 0));
    }
